...
def main():
    argument_spec = openstack_full_argument_spec(
        ...
        portsOnNetwork=dict(type='str', default=None)
    )
    ...
    jsonData = ast.literal_eval(module.params["portsOnNetwork"])
    module.params["portsOnNetwork"] = ""
    ...
    try:
        ...
        if not router:
            ...
            router_id = router['id']
            isChange = False
            for port in jsonData['ansible_facts']['openstack_ports']:
                try: 
                    cloud.remove_router_interface(router, port_id=port['id'])
                    isChange = True
                except:
                    # the port maybe removed
                    pass
